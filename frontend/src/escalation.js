...
export const EscalationList = props => (
    <List filters={<EscalationFilter />} filterDefaultValues={{ tasktypeId: "2" }} {...props}>
...
const EscalationTitle = ({ record }) => {
        return <span>Escalation {record ? `"${record.taskName}"` : ''}</span>;
    };
export const EscalationEdit = props => (
    <Edit title={<EscalationTitle />} {...props}>
        <SimpleForm>
